...
## 3.8.3 Spitzenwertgleichrichtung

Wenn man einen Kondensator parallel zum Verbraucher eines Gleichrichters anschließt, so kann man die entstehende Spannungskurve glätten. $u_{r}(t)$ ist die ungeglättete Kurve (Kapazität $C=0$), $u_{a}(t)$ die geglättete.

![[02 FÄCHER/ELEKTRONIK/attachments/03 Die Diode als Bauelement/file-20260506083635669.png]]
...
\large 
	...
	u_{a}(t)= \begin{cases}
	    u_{e}(t)-U_{D} & \text{wenn Diode leitet }  \\ 
	    u_{a}(t_{0}) \cdot e^{-\frac{t-t_{0}}{R\cdot C}} \approx u_{a}(t) \cdot \left( 1-\frac{{t-t_{0}}}{R\cdot C} \right) & \text{sonst} 
	\end{cases} 
$$
...
$$
u_{a}(t_{0})\bigg|_{C \to 0}^{}=\hat{u}_{a}\cdot\left( 1-\frac{U_{r}}{ \hat{u}_{a} } \right)
$$

folgt für kleine Winkel $\omega$

$$
	u_{a}(t_{1})\bigg|_{C\to 0}^{}\approx \hat{u}_{a}\cdot\left( 1-\frac{1}{2}(\omega \cdot\Delta t)^2 \right)
$$
und damit

$$
	\omega \cdot\Delta t\approx \sqrt{ \frac{2U_{r}}{ \hat{u}_{a}} }
$$

Für den Diodenstrom gilt

$$
	\overline{i}_{D}=\overline{i}_{R}\cdot \frac{T}{\Delta t}\approx \overline{i}_{R}\cdot \pi \sqrt{ \frac{2\hat{u}_{a}}{U_{r}} }
$$

$$
	\hat{\imath}_{D}=\overline{i}_{R}\left( \omega RC\cdot \sqrt{ \frac{2U_{r}}{ \hat{u}_{a}} }+1 \right)\approx \overline{i}_{R}\cdot 2\pi \cdot \sqrt{ \frac{{2 \hat{u}_{a}}}{U_{R}} }
$$

!TODO Spannungsvervielfachung

# 3.9 Die Schottky-Diode

Die Schottky-Diode ersetzt den p-Halbleiter mit einem Metall. Dadurch haben sie eine geringere Flussspannung ($0,3-0,5V$ bei Silizium) und schalten durch das Wegfallen der positiven Ladungen schneller um.

# 3.10 Die pin-Diode

Die pin-Diode hat einen Nichtleiter zwischen dem p-Halbleiter und n-Halbleiter. Das führt zu einer Zeitverzögerung beim Schalten, aber eine höhere Durchbruchspannung im Sperrbetrieb.

In der Praxis werden nur Dioden als pin-Dioden bezeichnet, die kurz bei negativer Spannung noch leiten. Bevor sie dann umschalten gilt
...
![[02 FÄCHER/ELEKTRONIK/attachments/03 Die Diode als Bauelement/file-20260506091957887.png]]

Z-Dioden sind für den Dauerbetrieb im Durchbruchbereich ausgelegt. Die Durchbruchspannung wird bei Z-Dioden oft als $U_{Z}$ bezeichnet und liegt typischerweise im Bereich $3V\dots 300V$.

![[02 FÄCHER/ELEKTRONIK/attachments/03 Die Diode als Bauelement/file-20260506091714023.png]]

Für das Ersatzschaltbild gilt

$$
	r_{z}=\frac{\Delta U_{Z}}{\Delta I_{Z}}
$$

Oberhalb eines bestimmten Kniestroms $I_{ZK}$ ist der Stromverlauf $I_{Z}$ im Durchbruchbereich annähernd linear.

Dioden mit $U_{Z}<5V$ haben wegen des Lawinen-Effekts einen positiven Temperaturkoeffizienten ($\uparrow T \to \uparrow V$). Dioden mit $U_{Z}>5V$ haben wegen des Zener-Effekts einen negativen Temperaturkoeffizienten.

Bei $U_{Z}\approx 5,1V$ ist der Temperaturkoeffizient $0mV/K$. Allerdings kann sich das für verschiedene $I_{Z}$-Werte ändern. Man kann auch 2 Dioden mit jeweils umgekehrten Temperaturkoeffizienten verwenden.

---

# Quellen

- [EK 1 Vorlesung 1 (J. Zwich, 2026)](https://elearning.dhbw-ravensburg.de/pluginfile.php/441396/mod_resource/content/1/Skript_EK1.pdf)
...
- [EK 1 Vorlesung 1 (J. Zwick, 2026)](https://elearning.dhbw-ravensburg.de/pluginfile.php/441396/mod_resource/content/1/Skript_EK1.pdf)
...
## [[#3.1 Definition und Kennlinie|3.1 Definition und Kennlinie]]
...
## [[#3.2 Sperrspannung|3.2 Sperrspannung]]
...
## [[#3.3 Durchlassbereich|3.3 Durchlassbereich]]
...
## [[#3.4 Temperaturabhängigkeit|3.4 Temperaturabhängigkeit]]
...
## [[#3.6.2 Wechselstromersatzschaltbild|3.6.2 Wechselstromersatzschaltbild]]
...
## [[#3.7.1 Gleichstrom|3.7.1 Gleichstrom]]
...
## [[#3.8.1 Der Einweggleichrichter|3.8.1 Der Einweggleichrichter]]
...
## [[#3.8.2 Der Brückengleichrichter|3.8.2 Der Brückengleichrichter]]
...
## [[#3.8.3 Spitzenwertgleichrichtung|3.8.3 Spitzenwertgleichrichtung]]
...
## [[#3.10 Die pin-Diode|3.10 Die pin-Diode]]

```latex
r_{D}=\frac{U_{T}}{I_{F}}
```

## [[#3.11 Die Z-Diode|3.11 Die Z-Diode]]
